...
;WITH TableResidents as (SELECT f.id as facilityId, f.facilityName, f.allResidential as allResidential, concat (p.firstName , ' ', p.middleName , ' ' , p.lastName) as name, p.lastName, p.BirthDate, bl.eventStart, bl.eventEnd,
...
ORDER BY [TableJoin].[facilityName] ASC, [TableJoin].[lastName] ASC