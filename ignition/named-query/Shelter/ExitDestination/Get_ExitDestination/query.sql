...
,bl.exit_ProviderId as providerId
,ISNULL((SELECT providerName FROM organization.Provider WHERE [organization].[Provider].id = bl.exit_ProviderId),'') as providerName
from
...
[TableResidents].isActive,
	...
	[TableResidents].providerId,
	[TableResidents].providerName,
...
AND TableJoin.[facilityName] in {shelter}
	...
	AND ( TableJoin.[providerId] = :providerId OR :providerId IS NULL)
...
AND (
	...
	OR [TableJoin].[providerName] LIKE CONCAT(:search_text,'%')
)
