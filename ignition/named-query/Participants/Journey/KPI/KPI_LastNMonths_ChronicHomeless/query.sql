...
SELECT
	MONTH(h.chronicHomelessDate) as monthValue, count(distinct h.ParticipantID) as countValue
FROM
...
WHERE
 	...
 	(h.chronicHomelessDate between DATEADD(month, -:monthCount, getDATE()) AND GETDATE()) AND
 	(:allSelection = 0 OR 
...
GROUP BY MONTH(h.chronicHomelessDate)
ORDER BY MONTH(h.chronicHomelessDate)
