...
SELECT
	MONTH(d.timeDeceased) as monthValue, count(distinct p.id) as countValue
FROM
...
WHERE
	d.timeDeceased is not null AND
 	...
 	(d.timeDeceased between DATEADD(month, -:monthCount, getDATE()) AND GETDATE()) AND
...
GROUP BY MONTH(d.timeDeceased)
ORDER BY MONTH(d.timeDeceased)
