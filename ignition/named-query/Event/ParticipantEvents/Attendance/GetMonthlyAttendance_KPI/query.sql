...
LEFT JOIN lodging.Facility f on r.facilityId = f.id
		JOIN participant.participant p ON ea.participantId = p.id
	 JOIN 	humans.human h ON p.humanId = h.id
	 JOIN 	humans.Gender g ON h.genderId=g.id
	WHERE 
		ea.timeRetired is NULL AND es.timeRetired is NULL
		AND (:facilitatorId = -1 OR :facilitatorId IS NULL OR :facilitatorId IN (SELECT  facilitatorHumanId FROM participant.EventSelectedFacilitators WHERE eventId = es.id ))
