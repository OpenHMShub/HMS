...
SELECT s1.locationId, s1.scheduleId, s1.seasonId, s1.totalBeds, s1.gender,s1.genderId, s1.dayOfYear,s1.DateFromDayOfYear,COALESCE(sp.totalParticipants,0),
CASE 
...
WHEN s1.timeCancelled IS NULL AND s1.timeCreated > @minimumTimeCreated and s1.timeCreated > datefromparts(@YearStart, 11,1) THEN 'Addition'
