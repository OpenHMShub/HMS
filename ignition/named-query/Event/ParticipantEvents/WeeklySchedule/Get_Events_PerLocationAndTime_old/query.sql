...
exec('SELECT 1 AS ''id'',concat(''' + @date + ''','' 07:00:00'') AS ''date'',''''As ''Day'', [TIME], ' + @location_list1 +' 
				FROM (
				  ...
				  FROM 
						...
						LEFT JOIN participant.Events e on l.id = e.[locationId]
						LEFT JOIN [participant].[EventSchedule] s on s.[eventId] = e.id
						LEFT JOIN [participant].[EventCategories] c on e.[categoryId] = c.[id]
						
						WHERE (e.[locationId] = ''' + @locationId + ''' OR ''' + @locationId + ''' = NULL OR ''' + @locationId + ''' = '''' OR ''' + @locationId + ''' = -1 )
